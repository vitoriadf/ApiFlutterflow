...
<div class="max-w-7xl mx-auto mt-12 mb-6 px-4">
    <div class="max-w-5xl mx-auto mt-12 mb-6 flex justify-between items-center">
        ...
        <div class="mt-2">
            <a href="{{ route('categorias.create') }}" class="text-blue-700 hover:text-white border border-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2 dark:border-blue-500 dark:text-blue-500 ">
                Adicionar
            </a>
        </div>
    </div>
    ...
    <div class="bg-fuchsia-100 shadow-md rounded-xl p-6 mt-12 max-w-5xl mx-auto">
        <h2 class="text-lg font-semibold text-fuchsia-900 mb-4">Lista de Categorias</h2>
        <table class="w-full text-left text-sm">
            ...
            <tbody>
                ...
                <tr class="border-b hover:bg-fuchsia-200 text-fuchsia-900 border-fuchsia-200"">
                    <td class="py-2 px-3">{{ $categoria->id }}</td>
                    <td class="py-2 px-3">{{ $categoria->nome }}</td>
                    <td class="py-2 px-3 text-center">
                        <div class="inline-flex space-x-4">
                            <a href="{{ route('categorias.edit', ['categoria' => $categoria->id]) }}" class="font-medium text-blue-600 hover:underline">Editar</a>
                            <a href="{{ route('categorias.destroy', $categoria->id) }}" class="font-medium text-red-600 hover:underline">Deletar</a>
                        </div>
                    </td>
                </tr>
                @endforeach
    ...
    <div class="relative p-4 w-full max-w-xl min-h-[400px]">
        <div class="relative bg-fuchsia-200 text-fuchsia-900 rounded-lg shadow-2xl">
            <div class="flex items-center justify-between p-4 border-b rounded-t dark:border-fuchsia-950">
                <h3 class="text-lg font-semibold">Adicionar Nova Categoria</h3>
                <form action="{{ route('categorias.closeModal') }}" method="POST" class="inline">
                    @csrf
...
@if(session('showCategoriaEditModal'))
<div class="fixed inset-0 z-50 flex justify-center items-center bg-gray-900 bg-opacity-50 z-50">
    <div class="relative p-4 w-full max-w-xl min-h-[400px]">
        <div class="relative bg-fuchsia-200 text-fuchsia-900 rounded-lg shadow">
            <div class="flex items-center justify-between p-4 border-b rounded-t dark:border-fuchsia-950">
                <h3 class="text-lg font-semibold">Editar Categoria</h3>
                <form action="{{ route('categorias.closeModalEdit') }}" method="POST" class="inline">
                    @csrf
                    <button type="submit" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 inline-flex justify-center items-center dark:hover:bg-fuchsia-900 dark:hover:text-white">
                        <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" />
            ...
            <form action="{{ route('categorias.update', session('categoria')->id) }}" method="POST" class="p-4 md:p-5">
                @csrf
                @method('PUT')
                <div class="grid gap-4 mb-4">
                    <div class="col-span-2">
                        <label for="nome" class="block mb-2 text-sm font-medium">Nome da Categoria</label>
                        <input type="text" name="nome" id="nome" value="{{ old('nome', session('categoria')->nome) }}" class="bg-fuchsia-200 border border-gray-400 text-fuchsia-900 text-sm rounded-lg focus:border-fuchsia-500 dark:focus:border-fuchsia-600  focus:outline-none focus:ring-2 dark:focus:ring-fuchsia-600  block w-full p-2.5 dark:bg-fuchsia-200 dark:border-fuchsia-300 dark:placeholder-fuchsia-400 dark:text-fuchsia-900" placeholder="Digite o nome da categoria">
                    </div>
                </div>
                <div class="flex justify-center space-x-4 mt-6">
                    <button type="submit" class="text-white items-center bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
...
@if (session('showCategoriaDeleteModal'))
<div id="popup-modal" tabindex="-1" class="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50 z-50">
    <div class="relative p-4 w-full max-w-md max-h-full">
        <div class="bg-fuchsia-200 rounded-lg shadow-2xl">
            <form action="{{ route('categorias.closeModalDelete') }}" method="POST" class="inline">
                @csrf
            ...
            <div class="p-4 text-center">
                ...
                <h3 class="mb-5 text-lg font-normal text-fuchsia-900">Você tem certeza que deseja excluir esta categoria?</h3>
                <form action="{{ route('categorias.confirmDelete', session('categoriaDeleteId')) }}" method="POST" class="inline">
                    @csrf
                    <button type="submit" class="text-white bg-red-600 hover:bg-red-800 px-5 py-2.5 rounded-lg text-sm">Confirmar</button>
                </form>
                <a href="{{ route('categorias.index') }}" class="py-2.5 px-5 ms-3 text-sm font-medium text-gray-900 bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:outline-none">
                    Cancelar
                </a>
            </div>
        </div>
